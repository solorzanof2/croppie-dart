...
import 'package:croppie_dart/src/croppie_dart_base.dart';

export 'src/croppie_dart_base.dart';
...
abstract class CroppieBase {
  ...
  ///     Valid Values: bool
  Promise result(String type, String size, String format, int quality, bool circle);
...
abstract class Croppie implements CroppieBase {
  factory Croppie(Element el, Options opts) {
    return new _Croppie(el, opts);
  }

  /// Simple async
  Future<T> resultAsync<T>({String type, String size, String format, int quality, bool circle});

  /// Async base64 encoded string
  Future<String> resultBase64({String size, String format, int quality, bool circle});

  /// Async html element
  Future<T> resultHtml<T extends Element>({String size, String format, int quality, bool circle});

  /// Async blob
  Future<Blob> resultBlob({String size, String format, int quality, bool circle});

  /// Async byte array
  Future<Uint8List> resultByteArray({String size, String format, int quality, bool circle});

  /// Async canvas element
  Future<CanvasElement> resultRawCanvas({String size, String format, int quality, bool circle});
...
class _Croppie implements Croppie {
  ...
  _Croppie(this.cropElement, this.opts) : impl = new CroppieJS(cropElement, opts);
  ...
  Future<Uint8List> resultByteArray({String size, String format, int quality, bool circle}) async {
    Blob blob = await resultBlob(size: size, format: format, quality: quality, circle: circle);

    Completer<Uint8List> out = new Completer();

    // Convert blob to Uint8List
    FileReader fr = new FileReader();
    fr.onLoadEnd.listen((ProgressEvent event) {
      out.complete(fr.result);
  ...
  @override
  Future<CanvasElement> resultRawCanvas({String size, String format, int quality, bool circle}) async {
